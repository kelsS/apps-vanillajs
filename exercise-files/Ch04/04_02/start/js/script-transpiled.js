...
(function () {
	...
	var apiKey = "9a3cbcffa8095bf058e5356342758144"; // Replace "APIKEY" with your own API key; otherwise, your HTTP request will not work
	var activities = {
	...
	function updateUISuccess(response) {
		...
		var container = document.createElement('div'); // container for weather data
		var cityPara = document.createElement('p'); // p for city info data
		...
		var conditionsPara = document.createElement('p'); // p for weather conditions data
		...
		conditionsPara.textContent = state.degCInt + "\xB0 C / " + state.degFInt + "\xB0 F"; // for degrees symbol
		// use unicode versions of html codes for &#176; when using vanilla js
		var iconImage = document.createElement('img'); // image element for weather icon
		...
		if (document.querySelector('.conditions div')) {
			//  this is true if that element exists
	...
	function updateActivityList(event) {
		...
		if (event !== undefined && event.target.classList.contains('selected')) {
			// event.target is a reference to the element that the event happened to
		...
		state.activities.forEach(function (activity, index) {
			// activity is the text of the li array item and index is the value of the index # for array element
